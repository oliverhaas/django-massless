"""Serve the current Django project under massless:
``python -m massless --settings myproject.settings --host H --port P
...
workers do NOT inherit this process's Django setup, so the worker target
re-bootstraps Django (from ``DJANGO_SETTINGS_MODULE``/``--settings``) and builds a
fresh handler before serving.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--settings",
        default=None,
        help="DJANGO_SETTINGS_MODULE to configure before serving",
    )
    return parser
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.processes <= 1:
        ...
        serve(handler, args.host, args.port, args.workers)
    else:
